...
- Exact-commit Codex-browser rehearsal of the earlier flow: the expected initial tool surface; four evidence-linked proposals staged; human accept/reject decisions; a reviewed state with the export tool unlocked; a downloadable receipt with decisions, locators, and audit events; one H1; no horizontal overflow at 1280 pixels; no browser warnings or errors.

...
| `28257` | Tested agents/clients | Codex desktop in-app browser with WebMCP site tools enabled (earlier flow, 2026-08-30/31); Google Chrome 152.0.7977.65 headless with `--enable-features=WebMCPTesting` driven by `npm run smoke:webmcp` (real-pair loop, 2026-09-01; registration-history loop against a local production build, 2026-09-02, with the production run pending). |

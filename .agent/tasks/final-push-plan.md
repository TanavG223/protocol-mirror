...
| # | Phase | Owner | Status |
|---|---|---|---|
| 0 | Commit Codex's 1080p60 render and media-contract updates; add this plan | Claude | in progress |
| 1 | `document.modelContext ?? navigator.modelContext`, console errors instead of silent catches, no tool count in preview mode | Claude | pending |
| 2 | Real pair becomes the reviewable case: "Review this pair" after both live fetches; pair-bound tools re-register on the new ids; honest labels (abstract sections, live provenance); 43-outcome layout; switch guard; enum threshold 20 | Claude | pending |
| 3 | Human-side loading: curated real pairs (ACTT-1 NCT04280705/32445440, Pfizer BNT162b2 NCT04368728/33301246, RECOVERY dexamethasone NCT04381936/32678530) as one-click chips plus an NCT/PMID form, so a judge in plain Chrome can load a real case without an agent | Claude | pending |
| 4 | Domain-first hero; CTA scrolls to the queue; "Uncertain" label; readable benchmark cards; `release-preflight.sh:85` grep updated | Claude | pending |
| 5 | ClinicalTrials.gov `?fields=` projection (fixes NCT04368728 502); upstream 400 → `invalid_identifier` | Claude | pending |
| 6 | `scripts/webmcp-smoke.mjs`: headless Chrome 152 + `--enable-features=WebMCPTesting` over CDP; drives `executeTool` through the full loop; asserts 6 → 7 → 6 and `generatedFrom: live_sources`; `npm run smoke:webmcp` | Claude | pending |

...
| 12 | Round three (d77e447, 437c1ce): critic blockers fixed — history sampling with per-change `exact`/`after`, `comparedVersions`/`unreadVersions`/`complete`, 8 MB per-version cap (Pfizer pair works), entity decoding, zero-original-primary note, opening-load race; PubMed `publishedOn` and "N of M changes predate the publication"; `get_audit_state` `outcomes: primary\|all`; reviewer mid-rejection keeps focus; production smoke PASS, Lighthouse 100/100/100/100; screenshots regenerated | Claude | done; round-four critic and competitor scan running |

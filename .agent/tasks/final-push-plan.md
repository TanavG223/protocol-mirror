...
| 9 | Final `npm run check`, honesty grep, push, live checks (badge, NCT04368728 200), hand-off report | Claude | done (6ff4a32): CI green, production smoke PASS, Lighthouse 100/100/100/100, NCT04368728 200; hand-off at `.agent/tasks/handoff-2026-09-01.md` |
| 10 | Round two (6921ea1): eighth tool `get_registry_history` (ClinicalTrials.gov registration versions; original primary outcomes merged into the reviewable pair with `history/0` locators); page opens on ACTT-1; reject-with-reason + notes to the agent (`reviewerFeedback`, `reviewerNotes` in `get_audit_state`); sessionStorage persistence + Clear session; `?nct=&pmid=` deep links; word-level span diff; smoke extended | Claude | done; copy updated (README, devpost, docs, packet check) |
| 12 | Round three (d77e447, 437c1ce): critic blockers fixed — history sampling with per-change `exact`/`after`, `comparedVersions`/`unreadVersions`/`complete`, 8 MB per-version cap (Pfizer pair works), entity decoding, zero-original-primary note, opening-load race; PubMed `publishedOn` and "N of M changes predate the publication"; `get_audit_state` `outcomes: primary\|all`; reviewer mid-rejection keeps focus; production smoke PASS, Lighthouse 100/100/100/100; screenshots regenerated | Claude | done |
| 13 | Round four: measure-only history comparison with separate time-frame edits; truncated notes; stale rejection reason cleared; hedged predate count; session-shape validation + error boundary; copy corrections. Competitor scan (`.agent/tasks/codex-briefing-2026-09-02.md` summarises the consequences): the human-gate pitch is crowded, live data + time dimension is unique, an origin-trial token would remove the Chrome flag for judges (Tanav must register it) | Claude | done; round five PASS on all four lenses (text blockers B1–B4 fixed) |
| 11 | `load_trial_pair` as an agent-callable case switch | Claude | dropped: the human click on "Review this pair" is the authority moment worth keeping |
...
- `src/lib/source-adapters.ts` `fetchRegistryHistory` + route `/api/clinical-trials/[nctId]/history`: version list (`/api/int/studies/<NCT>/history`), fetches version 0 plus up to 6 versions whose `moduleLabels` include "Outcome Measures", diffs primary outcome sets, returns original/timeline/firstPrimaryChange. `src/lib/live-pair.ts` merges original primaries as `registry-original-primary-N`.
...
| 14 | UI polish (eed1d5d, 24b0266, 74622c3): three-lens design critique + reference-repo distillation → responsive fixes at 390–1280 px, motion tokens and CSS-first entrance (GSAP removed), pointer spotlight/sheen/badge ring/unlock ink, depth and contrast fixes, on-page 'Ask your agent' prompt panel; production smoke PASS, Lighthouse 100s | Claude | done; UI frozen for the recording |

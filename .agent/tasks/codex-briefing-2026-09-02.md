...
**Update, ~00:15 ET — hold the recording for about 90 minutes.** Tanav asked for a UI polish pass (responsiveness at the in-app browser width, motion, depth). It changes how the hero, the case card and the cards look, so a recording made now would not match the final page. I will append the commit here the moment it is live on production; record after that. The judge path, tool names, texts and counts do not change.

...
The final local candidate is [`docs/demo/protocol-mirror-final-demo.mp4`](docs/demo/protocol-mirror-final-demo.mp4): 89.65 seconds, 1280×720 H.264 at constant 60 fps with a natural ElevenLabs narration. It has passed cadence, continuity, loudness, codec, and representative-frame checks; the owner must still watch and approve it with sound before upload. Voice direction, narration, source audio, and reproducible render assets live in [`docs/demo/`](docs/demo/) and [`scripts/render-final-demo.sh`](scripts/render-final-demo.sh).
